...
@section('title', 'HRMS | إضافة تهيئة جديدة')
...
<div style="margin-bottom: 20px;">
    <h2 style="margin: 0 0 10px 0;">إضافة تهيئة جديدة</h2>
    <p style="color: #6b7280; margin: 0;">قم بإدخال بيانات التهيئة للموظف الجديد</p>
</div>
...
<form action="{{ route('onboardings.store') }}" method="POST">
    ...
    <div class="form-group">
        ...
        <textarea name="tasks" placeholder="أدخل المهام المطلوبة (سطر واحد لكل مهمة)">{{ old('tasks') }}</textarea>
        @error('tasks')
            <span style="color: #ef4444; font-size: 13px;">{{ $message }}</span>
        @enderror
    ...
    <div class="form-group">
        ...
        <textarea name="documents" placeholder="أدخل الوثائق المطلوبة (سطر واحد لكل وثيقة)">{{ old('documents') }}</textarea>
        @error('documents')
            <span style="color: #ef4444; font-size: 13px;">{{ $message }}</span>
        @enderror
