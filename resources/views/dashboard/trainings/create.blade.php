...
@section('title', 'HRMS | إضافة دورة جديدة')
...
<div style="margin-bottom: 20px;">
    <h2 style="margin: 0 0 10px 0;">إضافة دورة جديدة</h2>
    <p style="color: #6b7280; margin: 0;">قم بإدخال بيانات الدورة التدريبية</p>
</div>
...
<form action="{{ route('trainings.store') }}" method="POST">
    ...
    <div class="form-group">
        ...
        <input type="text" name="title" value="{{ old('title') }}" required placeholder="أدخل عنوان الدورة">
        @error('title')
    ...
    <div class="form-group">
        ...
        <textarea name="description" required placeholder="أدخل وصف الدورة">{{ old('description') }}</textarea>
        @error('description')
            <span style="color: #ef4444; font-size: 13px;">{{ $message }}</span>
        @enderror
    </div>
    ...
    <div class="form-group">
        <label>المدرب</label>
        <input type="text" name="instructor" value="{{ old('instructor') }}" placeholder="أدخل اسم المدرب">
        @error('instructor')
            <span style="color: #ef4444; font-size: 13px;">{{ $message }}</span>
        @enderror
    </div>
    ...
    <div class="form-group">
        ...
        <textarea name="participants" placeholder="أدخل قائمة المشاركون (JSON format)">{{ old('participants') }}</textarea>
        @error('participants')
            <span style="color: #ef4444; font-size: 13px;">{{ $message }}</span>
        @enderror
    ...
    <div class="form-group">
        ...
        <textarea name="skills_covered" placeholder="أدخل المهارات المغطاة في الدورة">{{ old('skills_covered') }}</textarea>
    ...
    <div class="form-actions">
        ...
        <a href="{{ route('trainings.index') }}" class="btn btn-back"> رجوع</a>
    </div>
</form>
@endsection
